...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    val l: Listener = createListener(relativePath)
    l.start()

    Future.run()(t => future {
      while (t.nonCancelled) {
        val requestFuture: Future[(NodeScala.Request, Exchange)] = l.nextRequest()
        requestFuture onComplete {
          case Success(rAndE) => {
            val response: NodeScala.Response = handler(rAndE._1)
            respond(rAndE._2, t, response)
          }
          case _ =>
        }
      }
