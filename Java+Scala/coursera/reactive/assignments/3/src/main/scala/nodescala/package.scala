...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def never[T]: Future[T] = {
      future {
        Observable.interval(1000 day).take(1).toBlockingObservable.toList
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] =
      future {
        fs.map(f => Await.result(f, 1000 day)).toList
      }
    ...
    def delay(t: Duration): Future[Unit] = future {
      Observable.interval(t).take(1).toBlockingObservable.toList
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val t: CancellationTokenSource = CancellationTokenSource()
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continue[S](cont: Try[T] => S): Future[S] = future {
      val result: T = Await.result(f, 10 second)
      cont(Try(result))
    }
  }
